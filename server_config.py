...
def generate_config(instances, vsize, slab_mem):
  ...
  # create slimcache config file(s)
  for i in range(instances):
    admin_port = PELIKAN_ADMIN_PORT + i
    server_port = PELIKAN_SERVER_PORT + i
    pmem_nr = 12 + i%2
    config_file = 'slimcache-{server_port}.config'.format(server_port=server_port)
    config_str = """\
daemonize: yes
...
debug_log_level: 5
debug_log_file: log/slimcache-{server_port}.log
debug_log_nbuf: 1048576

klog_file: log/slimcache-{server_port}.cmd
klog_backup: log/slimcache-{server_port}.cmd.old
klog_sample: 100
klog_max: 1073741824
...
request_poolsize: 100000
response_poolsize: 200000

time_type: 2
cuckoo_datapool: /mnt/pmem{pmem_nr}/pool{server_port}
cuckoo_item_size: {item_size}
cuckoo_nitem: {nkey}
""".format(i=i, admin_port=admin_port, server_port=server_port, vsize=vsize, nkey=nkey, hash_power=hash_power, slab_mem=slab_mem, item_size=item_size, pmem_nr=pmem_nr)
...
def generate_runscript(binary, instances):
  ...
  with open(fname, 'w') as the_file:
    for i in range(instances):
      config_file = os.path.join('config', 'slimcache-{server_port}.config'.format(server_port=PELIKAN_SERVER_PORT+i))
      if BIND_TO_NODES:
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="""
    ...
    """)
  parser.add_argument('--binary', dest='binary', type=str, help='location of pelikan_slimcache binary', required=True)
